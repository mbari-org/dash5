...
const server = setupServer(
  ...
  rest.get('/events', (req, res, ctx) => {
    // Return timeout note when the dedicated timeout-notes sub-query fires
...
describe('CommsSection', () => {
  ...
  test('overrides queued/sent status to timeout when internal timeout-notes query returns a matching note', async () => {
    ...
    // A timeout note referencing that eventId is returned by the timeout-notes
    // query, so timedOutMap should override the status to 'timeout' and the
    // cell should display the note's timestamp instead of the queued time.
    ...
    server.use(
      rest.get('/events', (req, res, ctx) => {
        ...
        // Return a single queued command (no sbdSend, no sbdReceipt events)
        return res(
          ctx.status(200),
          ctx.json({
            result: [
