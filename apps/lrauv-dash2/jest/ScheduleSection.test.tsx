...
test('cancelled directive stays cancelled after the original timeout expires', async () => {
  ...
  const cancellationNote = {
    ...
    data: null,
    text: null,
    note: `Cancelled request ${eventId} for 'example': 'load Science/profile_station.tl;run'`,
    user: 'test-operator',
  }
  const timeoutNote = {
    eventId: 86803,
    eventType: 'note',
    unixTime: sentAt + 5 * 60 * 1000,
    isoTime: new Date(sentAt + 5 * 60 * 1000).toISOString(),
    data: null,
    text: null,
    note: `id=${eventId}: Timeout while waiting for ack`,
    user: null,
