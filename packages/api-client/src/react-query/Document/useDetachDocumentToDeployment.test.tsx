...
import { useDetachDocumentToDeployment } from './useDetachDocumentToDeployment'
...
const MockAlterAction: React.FC = () => {
  const { mutate, isLoading, data } = useDetachDocumentToDeployment()
  const handleClick = () => {
    mutate({
      docId: 3000788,
  ...
  return isLoading ? null : (
    <div>
      <button onClick={handleClick}>Detach Deployment</button>
      {data && <span data-testid="result">{data?.docId}</span>}
    </div>
  )
}

describe('useDetachDocumentToDeployment', () => {
  it('should render the result upon submission', async () => {
    ...
    fireEvent.click(screen.getByText('Detach Deployment'))
    await waitFor(() => {
