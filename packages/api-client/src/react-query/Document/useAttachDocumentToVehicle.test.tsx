...
import { useAttachDocumentToVehicle } from './useAttachDocumentToVehicle'
...
const MockAlterAction: React.FC = () => {
  const { mutate, isLoading, data } = useAttachDocumentToVehicle()
  const handleClick = () => {
    mutate({
      docId: 3000788,
      vehicleName: 'sim',
  ...
  return isLoading ? null : (
    <div>
      <button onClick={handleClick}>Attach Vehicle</button>
      {data && <span data-testid="result">{data?.docId}</span>}
    </div>
  )
}

describe('useAttachDocumentToVehicle', () => {
  it('should render the result upon submission', async () => {
    ...
    fireEvent.click(screen.getByText('Attach Vehicle'))
    await waitFor(() => {
