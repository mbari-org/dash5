...
describe('extractPicAndOnCallFromNotes', () => {
  it('should return null for both pics and onCalls when no events are provided', () => {
    const result = extractPicAndOnCallFromNotes([])
    expect(result).toEqual({ pics: null, onCalls: null })
  })
  ...
  it('should correctly extract PIC and On-Call information from note events', () => {
    ...
    expect(result.pics).toEqual([
      {
        user: 'Bob',
        unixTime: 4000,
      },
    ])
    expect(result.onCalls).toEqual([
      {
        user: 'Jane',
        unixTime: 2000,
      },
  ...
  it('should handle multiple active PIC and On-Call operators', () => {
    const noteEvents: GetEventsResponse[] = [
      {
        eventId: 1,
        vehicleName: 'vehicle1',
